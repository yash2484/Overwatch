...
## 3. OSINT Fusion — DECIDED: IN MVP, constrained (2026-07-02)

v0.1 includes the news/OSINT correlation layer, under these constraints:

- **One source: GDELT** (free, natively geotagged via GKG). Direct API, no MCP.
...
- **Three-gate AND scorer (conservative by construction):** spatial (article geotag within AOI buffered 25 km) AND temporal (article date in [detection window start − 30 d, window end + 14 d]) AND thematic (per-vertical GDELT theme/keyword allowlist). Fail any gate → no citation. Zero matches → the brief simply has no news section. Better to cite nothing than cite garbage.
- **Observed vs. reported:** claims backed only by articles must use reported-speech framing ("regional news reports…"); the brief validator enforces it (§4).
- **Kill-switch:** fusion sits behind a config flag; if correlation quality disappoints, the demo ships imagery-only with citation slots intact.
- **Spike first:** the exact GDELT API surface (DOC 2.0 vs GEO 2.0, theme taxonomy) is verified empirically at the start of the fusion phase (§9 Phase 5) before any integration code.
...
- **Brief Generator:** LLM receives structured detections (not pixels) and writes the analyst brief. **Hard rule: every quantitative/temporal claim must carry evidence-link IDs; a post-generation validator rejects briefs containing unlinked claims; claims backed only by news articles (not detections) must use reported-speech framing.** This validator is the anti-hallucination gate and a first-class feature.
...
| News fusion | **GDELT** (DOC/GEO 2.0, spike-verified) | Free, natively geotagged — de-risks correlation. Direct API, no MCP. |
...
- **"How do you stop the LLM hallucinating in briefs?"** — It never sees pixels, only structured detections; the validator rejects unlinked claims; regeneration is bounded. The brief is narration over verified events, and you can prove it in the demo by clicking any sentence.
